...
import { aggregateDailyRefuels, buildFleetFuelPeriods } from './services/fuelPeriods'
...
// pleins du même camion à la même date sont agrégés en un seul événement par
// aggregateDailyRefuels avant de former les cycles (jamais scindés). `dh`
// inclut l'AdBlue acheté sur ce même plein de clôture (adblue_total, 0 si
// aucun AdBlue) — voir dieselDh/adblueDh pour le détail.
export function buildFuelCycles(gasoil) {
  ...
  Object.entries(byCamion).forEach(([camionId, entries]) => {
    const sorted = aggregateDailyRefuels(entries)
