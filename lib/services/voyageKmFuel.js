...
export function buildVoyageKmFuelTimeline({ voyages, camions, gasoil, voyageGasoilRows, remiseRate = DEFAULT_REMISE_CARBURANT_RATE }) {
  ...
  const rows = active.map(v => {
    ...
    const { cost: fuelCost, source: fuelSource } = computeFuelCost(v, table?.voyageFuelMap, voyageGasoilRows)
    ...
    const isPending = !!table?.pendingVoyageIds?.has(v.id)
    const status = deriveVoyageStatus({ hasCamion: !!v.camion_id, odo, fuelSource, isPending })
...
export function deriveVoyageStatus({ hasCamion, odo, fuelSource, isPending = false }) {
  ...
  if (fuelSource === 'none' && isPending) return 'pending_measurement'
