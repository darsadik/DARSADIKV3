...
import { buildTruckFuelPeriods, truckHistoricalConsumption, truckHistoricalCostPerKm } from './fuelPeriods'
...
export const TIMELINE_STATUS = {
  ...
  // Real refueling-period model (§7): this voyage sits after the truck's
  // most recent full plein — its real fuel cost isn't fabricated, it becomes
  // known the moment a later full plein closes that period.
  pending_measurement:     { emoji: '⏳', label: 'En attente du prochain plein',      tone: 'info' },
  ...
  // Display-only metadata for the provisional-estimate case (see
  // buildVoyageKmFuelTimeline's isProvisional below) — NEVER stored as a
  // row's own `status`, which stays exactly 'pending_measurement' for these
  // voyages so every existing status==='pending_measurement' filter
  // (fuelAllocationCenter.js's buildUnallocatedVoyages, TruckPlanCenter,
  // dashboards) keeps seeing them unchanged. Callers that want to show the
  // softer "estimated" wording check `row.isProvisional` and look this key up
  // explicitly instead of `TIMELINE_STATUS[row.status]`.
  provisional_measurement: { emoji: '≈', label: 'Provisoire — basé sur moyenne camion', tone: 'provisional' },
}
...
export function buildVoyageKmFuelTimeline({ voyages, camions, gasoil, voyageGasoilRows, remiseRate = DEFAULT_REMISE_CARBURANT_RATE }) {
  ...
  // Truck-specific historical average — provisional-estimate support only
  // (see isProvisional below). Built from lib/services/fuelPeriods.js's own
  // closedPeriods (the authoritative REFUEL→VOYAGES→NEXT REFUEL boundaries),
  // NOT from tableByCamion above — a separate, read-only computation that
  // never feeds into the real cost/allocation engine. Same once-per-truck
  // pattern as tableByCamion.
  const periodsByCamion = new Map()
  voyagesByCamion.forEach((camionVoyages, camionId) => {
    const { closedPeriods } = buildTruckFuelPeriods({
      camionGasoil: refillsByCamion.get(camionId) || [],
      camionVoyages,
    })
    periodsByCamion.set(camionId, {
      avgL100: truckHistoricalConsumption(closedPeriods),
      avgCoutKm: truckHistoricalCostPerKm(closedPeriods),
    })
  })
  ...
  const rows = active.map(v => {
    ...
    const { cost: fuelCost, source: fuelSource } = computeFuelCost(v, table?.voyageFuelMap, voyageGasoilRows, table?.pendingVoyageIds)
    ...
    if (contributions.length > 0) {
      ...
      // resolvedBy === 'date' (fuelAllocation.js's Priority 2 fallback) means
      // this purchase had no KM of its own — the period/voyage relationship
      // was established by chronology, not odometer. Surfaced here so the
      // Chronologie tab stays transparent about it (spec item 12) instead of
      // looking identical to a normal KM-measured plein.
      fillLabel = contributions.length === 1
        ? `Plein du ${contributions[0].date || ''}${contributions[0].resolvedBy === 'date' ? ' (KM manquant → associé par date)' : ''}`.trim()
        : `${contributions.length} pleins liés`
    }

    const status = deriveVoyageStatus({ hasCamion: !!v.camion_id, odo, fuelSource })

    // ── Provisional estimate — temporary, display-only stand-in for a
    // voyage whose real fuel cost isn't measurable yet (source === 'pending':
    // real km, automatic mode, no manual override, sitting in the truck's
    // still-open refueling period). Never replaces `status`/`fuelCost`/
    // `litersLinked` above (those stay exactly what the authoritative engine
    // says — 0/pending), never written back anywhere, and automatically
    // stops applying the moment a later Plein closes the period (fuelSource
    // becomes 'automatic'/'manuel' on the next read, since nothing here is
    // persisted). Camions Loué never qualify (§9/§14 case 10) — a missing/
    // null type_camion defaults to Propre, matching every other Propre-only
    // check in the app (fleetFuelMonitoring.js, pages/camions/[id].js).
    const isPropre = (camion?.type_camion) !== 'loue'
    const truckAvg = periodsByCamion.get(v.camion_id) || null
    const canEstimate = isPropre && fuelSource === 'pending' && distance !== null && distance > 0 &&
      truckAvg?.avgL100 !== null && truckAvg?.avgL100 !== undefined
    const isProvisional = canEstimate
    const provisionalL100 = canEstimate ? truckAvg.avgL100 : null
    const provisionalLiters = canEstimate ? Math.round(distance * truckAvg.avgL100 / 100 * 100) / 100 : null
    const provisionalCost = canEstimate && truckAvg.avgCoutKm !== null && truckAvg.avgCoutKm !== undefined
      ? Math.round(distance * truckAvg.avgCoutKm * 100) / 100
      : null
    ...
    return {
      ...
      fuelCost, costPerKm,
      isProvisional, provisionalL100, provisionalLiters, provisionalCost,
...
export function deriveVoyageStatus({ hasCamion, odo, fuelSource }) {
  ...
  if (fuelSource === 'pending') return 'pending_measurement'
...
// Originally checked whether Σ voyage_gasoil.total (an amount independently
// typed per link) exceeded its purchase's own total — exactly the
// double-counting bug the engine redesign eliminated (see
// sql/22_voyage_gasoil_pure_mapping.sql). Under the current purchase-scoped
// engine that condition is now mathematically impossible: a purchase's
// allocated total can never exceed its own total (distributeFuelPurchase's
// largest-remainder split, and resolveFuelPurchaseAllocation's proportional
// capping of overflowing manual_fixed requests both guarantee it). Reading
// voyage_gasoil.total here would only ever see 0 for links created under this
// engine (see lib/services/voyage/gasoilLink.js) — dead logic that can no
// longer detect anything real.
//
// What CAN still legitimately happen — and is worth surfacing — is exactly
// that capping: a manual_fixed voyage whose requested manual_fuel_cost
// couldn't be fully honored by its purchase and got scaled down. `voyageRows`
// (built once per truck by buildVoyageKmFuelTimeline, above) already carries
// each voyage's real allocated amount (fuelCost) straight from the engine —
// no new calculation, no new Supabase call, just reading what the engine
// already produced instead of the legacy column.
export function detectFuelAssignmentProblems({ voyages, voyageRows }) {
  const problems = []
  const voyageById = new Map((voyages || []).map(v => [v.id, v]))

  ;(voyageRows || []).forEach(row => {
    const v = voyageById.get(row.voyageId)
    if (!v || (v.fuel_mode || 'automatic') !== 'manual_fixed') return
    const requested = parseFloat(v.manual_fuel_cost) || 0
    const received = row.fuelCost || 0
    if (requested > received + 0.01) {
      const gasoilId = row.contributions?.[0]?.gasoilId ?? null
      const purchaseDate = row.contributions?.[0]?.date
      problems.push({
        severity: 'warning', type: 'fixed_amount_capped',
        camionId: row.camionId,
        message: `${row.reference} (${row.plaque}) : montant fixe demandé de ${fmtMoney(requested)} DHS réduit à ${fmtMoney(received)} DHS${purchaseDate ? ` — plein du ${purchaseDate} insuffisant` : ' — plein insuffisant'}.`,
        gasoilId, voyageIds: [row.voyageId],
      })
    }
  })
