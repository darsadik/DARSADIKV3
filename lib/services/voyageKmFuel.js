...
export const TIMELINE_STATUS = {
  ...
  fuel_missing:      { emoji: '🟡', label: 'Carburant non assigné',   tone: 'warning' },
  cannot_calculate:  { emoji: '🔴', label: 'Calcul impossible',       tone: 'error' },
}
...
export function buildVoyageKmFuelTimeline({ voyages, camions, gasoil, voyageGasoilRows, remiseRate = DEFAULT_REMISE_CARBURANT_RATE }) {
  ...
  const rows = active.map(v => {
    ...
    let status
    if (!v.camion_id) status = 'cannot_calculate'
    else if (!odo) status = 'cannot_calculate'
    else if (odo.status === 'broken_chronology') status = 'cannot_calculate'
    else if (odo.status === 'missing_km') status = 'missing_start_km'
    else if (odo.status === 'waiting') status = 'missing_end_km'
    else if (fuelSource === 'none') status = 'fuel_missing'
