    ...
    const cycles = groups.map((group, i) => {
      ...
      if (!isOpen) {
        ...
        return {
          ...
          // Display-only (spec §5): the refill that closed this cycle is the
          // NEXT group's first plein — it's never part of `pleins` itself,
          // since it belongs to the following cycle. Purely descriptive,
          // doesn't affect kmFin/distance/cost math above.
...
export function detectMissingData({ byCamion, gasoil = [], voyages = [] }) {
  ...
  byCamion.forEach(({ camionId, camionPlaque, gasoil, voyages }) => {
    ...
    // Same truck, same day, ≥2 pleins, at least one missing heure — only
    // flagged when it's actually ambiguous which came first, since `heure`
    // is optional and rarely filled otherwise (would flood the list).
    const byDate = {}
    gasoil.forEach(g => { (byDate[g.date] = byDate[g.date] || []).push(g) })
    Object.entries(byDate).forEach(([date, rows]) => {
      if (rows.length > 1 && rows.some(g => !g.heure)) {
        rows.filter(g => !g.heure).forEach(g => {
          push('plein_missing_hour', 'info', camionId, camionPlaque, 'plein', g.id, date,
            `Plein du ${date} (${camionPlaque}) sans heure — ${rows.length} pleins ce jour-là, ordre ambigu.`)
        })
      }
    })
