...
//    pair could be split by the cross-date merge logic below. A Bon with NO
//    KM can never itself be a boundary (see isRefuelRow), but unlike
//    fuelPeriods.js — the shared, money-engine-facing module this one may
//    not touch — its litres are never dropped here either: they carry
//    forward (carryQte/carryTotal/carryRows below) into whichever later
//    day-event for this SAME truck DOES have a valid KM, exactly like
//    fleetFuelMonitoring.js's Bon-table carry-forward. This module is
//    display/analysis only (never feeds profitability — see file header),
//    so this carry-forward is safe and self-contained.
...
// summed across every underlying row in the group PLUS any carried-forward
// missing-KM litres — this group's own totals are what CLOSES the previous
// cycle (see buildFuelCycles below), not what funds the voyages that follow
// it. `pleins` itself only ever holds real KM-bearing rows (the "Plein
// d'ouverture"/"Plein intermédiaire" list in CycleCard, and the
// merge_with_previous heuristic below, both depend on that) — carried rows
// are exposed separately as `carriedRows` so their litres are visible, not
// silently folded into a list that assumes every entry has its own KM.
export function groupPleinsIntoCycles(gasoilForCamion, voyagesForCamion) {
  const relevant = (gasoilForCamion || []).filter(g => (g.qte || 0) > 0).sort(comparePleins)

  const dayEvents = []
  let carryQte = 0, carryAdblueQte = 0, carryTotal = 0, carryAdblueTotal = 0, carryRows = []
  const dropCarry = day => {
    day.carriedQte += carryQte; day.carriedAdblueQte += carryAdblueQte
    day.carriedTotal += carryTotal; day.carriedAdblueTotal += carryAdblueTotal
    day.carriedRows.push(...carryRows)
    carryQte = 0; carryAdblueQte = 0; carryTotal = 0; carryAdblueTotal = 0; carryRows = []
  }
  relevant.forEach(g => {
    if (!hasKm(g)) {
    ...
    if (last && last.date === g.date) {
      last.pleins.push(g)
    } else {
      dayEvents.push({ date: g.date, pleins: [g], carriedQte: 0, carriedAdblueQte: 0, carriedTotal: 0, carriedAdblueTotal: 0, carriedRows: [] })
    }
    dropCarry(dayEvents[dayEvents.length - 1])
  })
  // Trailing missing-KM Bons with no later KM-bearing Bon yet are deliberately
  // left un-folded here — the day-event they'd close doesn't exist yet, and
  // folding them onto the LAST already-known day would retroactively inflate
  // that day's own already-closed cycle with litres it never measured (the
  // same "never fabricate the open period" rule as fuelPeriods.js's
  // openPeriod). They stay individually visible via detectMissingData below
  // regardless, and will correctly fold into the real closing Bon the moment
  // one is entered — everything here is recomputed live, nothing persisted.
  ...
  return groups.map(group => {
    ...
    const carriedRows = group.days.flatMap(d => d.carriedRows)
    ...
    return {
      ...
      litresGasoil: pleins.reduce((s, p) => s + (p.qte || 0), 0) + group.days.reduce((s, d) => s + d.carriedQte, 0),
      litresAdblue: pleins.reduce((s, p) => s + (p.adblue_qte || 0), 0) + group.days.reduce((s, d) => s + d.carriedAdblueQte, 0),
      montantGasoil: pleins.reduce((s, p) => s + (p.total || 0), 0) + group.days.reduce((s, d) => s + d.carriedTotal, 0),
      montantAdblue: pleins.reduce((s, p) => s + (p.adblue_total || 0), 0) + group.days.reduce((s, d) => s + d.carriedAdblueTotal, 0),
  ...
  byCamion.forEach(({ camionId, camionPlaque, gasoil, voyages }) => {
    ...
    gasoil.filter(g => !hasKm(g)).forEach(g => {
      push('plein_missing_km', 'warning', camionId, camionPlaque, 'plein', g.id, g.date,
        `Plein du ${g.date} (${camionPlaque}) sans KM compteur — ses litres seront comptés avec le prochain plein daté.`)
    })
