...
// Starting cash balance for the Caisse module, representing whatever
// physical company cash existed before Caisse tracking began. Entered once
// on the /caisse page — never a movement itself, see sql/25_caisse.sql.
export const DEFAULT_CAISSE_SOLDE_INITIAL = 0

export async function fetchCaisseSoldeInitial() {
  const { data } = await supabase.from('app_settings').select('value').eq('key', CAISSE_SOLDE_INITIAL_KEY).maybeSingle()
  const v = parseFloat(data?.value)
  return Number.isFinite(v) ? v : DEFAULT_CAISSE_SOLDE_INITIAL
}

export async function saveCaisseSoldeInitial(value) {
  return supabase.from('app_settings').upsert({
    key: CAISSE_SOLDE_INITIAL_KEY,
    value: String(value),
    updated_at: new Date().toISOString(),
  })
}
