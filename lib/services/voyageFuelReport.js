...
export function voyageReportStatus(row) {
  ...
  // Provisional estimate (lib/services/voyageKmFuel.js's isProvisional) —
  // a truck-average-based stand-in shown ONLY while this voyage's real cost
  // is still unmeasured. Checked ahead of the plain 'pending_measurement'
  // branch below, which stays for voyages with no valid truck average yet.
  if (row.status === 'pending_measurement' && row.isProvisional) {
    return { text: 'Provisoire — basé sur moyenne camion', tone: 'provisional' }
  }
