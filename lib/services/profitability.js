...
// same distribution formula either way, never modified here). This module
// only picks which of the voyage's fuel_mode branches applies and, for
// automatic/manual_km/manual_fixed, looks this voyage's share up in that
// engine's output.
import { buildCamionFuelAllocationTable, distributeFuelPurchase } from './fuelAllocation'

// Builds every truck's allocation table ONCE and keeps only the voyageId→amount
// lookup — the single place the (potentially expensive, whole-history) engine
// build happens per request. Callers that need N voyages' fuel costs call this
// once, then do N cheap Map lookups via computeFuelCost, instead of N separate
// full rebuilds. Same grouping-by-camion_id pattern already used by
// lib/services/voyage/fuelAllocationCenter.js's buildAllocationCards (which
// stays untouched and keeps building its own, richer per-purchase view the
// same way it always has — this is a second, independent caller of the same
// unmodified engine function, not a shared cache).
export function buildFuelMapsByCamion({ gasoil, voyages, voyageGasoilLinks, remiseRate = DEFAULT_REMISE_CARBURANT_RATE }) {
  const gasoilByCamion = new Map()
  ;(gasoil || []).forEach(g => {
    if (g.camion_id === null || g.camion_id === undefined) return
    if (!gasoilByCamion.has(g.camion_id)) gasoilByCamion.set(g.camion_id, [])
    gasoilByCamion.get(g.camion_id).push(g)
  })
  const voyagesByCamion = new Map()
  ;(voyages || []).forEach(v => {
    if (v.camion_id === null || v.camion_id === undefined) return
  ...
  camionIds.forEach(camionId => {
    ...
    result.set(camionId, table.voyageFuelMap)
  })
  return result
}

// Voyages whose real fuel cost is not yet known (real km, automatic mode, no
// manual override, sitting in a truck's currently-open refueling period —
// see fuelAllocation.js's buildCamionFuelAllocationTable / fuelPeriods.js).
// A SEPARATE call from buildFuelMapsByCamion above on purpose: that function
// is called from several places (dashboard, review, voyages list) that don't
// need this distinction and must keep behaving exactly as before; only
// Rentabilité (computeVoyageProfit) needs it, so this stays additive and
// opt-in rather than changing buildFuelMapsByCamion's existing return shape.
// Same per-camion grouping, same unmodified engine call — no new math.
export function buildPendingFuelVoyageIds({ gasoil, voyages, voyageGasoilLinks, remiseRate = DEFAULT_REMISE_CARBURANT_RATE }) {
  const gasoilByCamion = new Map()
  ;(gasoil || []).forEach(g => {
    if (g.camion_id === null || g.camion_id === undefined) return
    if (!gasoilByCamion.has(g.camion_id)) gasoilByCamion.set(g.camion_id, [])
  ...
  camionIds.forEach(camionId => {
    ...
    table.pendingVoyageIds.forEach(id => pendingVoyageIds.add(id))
  })
  return pendingVoyageIds
}
...
// DHS slice) — reads its share straight out of `voyageFuelMap`, which the
// caller built ONCE (via buildFuelMapsByCamion, above) for the whole truck
// rather than rebuilding it here per voyage. A voyage's total cost is the
// sum of its shares across every purchase it appears in (normally one, but
// legitimately more than one if it refueled mid-trip via manual links) —
// that summation already happened when the map was built.
//
// `pendingVoyageIds` (optional — see buildPendingFuelVoyageIds above) is
// what turns a bare cost=0 into a real distinction: a voyage with real KM
// sitting in a truck's still-open refueling period is NOT "no fuel cost", it
// is "not measured yet" — source:'pending' — never the same as source:'none'
// (a voyage that genuinely has no camion/km/purchase to draw from at all).
// Callers that don't pass pendingVoyageIds (dashboard, review, voyages list —
// none of which need this distinction) keep getting 'none' exactly as
// before; this is purely additive.
export function computeFuelCost(voyage, voyageFuelMap, voyageGasoilLinks, pendingVoyageIds) {
  ...
  const cost = Math.round((voyageFuelMap?.get(voyage.id) || 0) * 100) / 100
  if (cost <= 0) {
    if (pendingVoyageIds?.has(voyage.id)) return { cost: 0, source: 'pending' }
...
export function computeVoyageProfit({
  ...
  voyageFuelMap = new Map(), voyageGasoilLinks = [], pendingVoyageIds = new Set(),
}) {
  ...
  // frais_total is NET (Σ charges − Σ deductions, lib/services/voyage/livraisons.js
  // computeFrais) — correct for what's billed to the client, but a déduction
  // ("Transport payé": client already paid that expense directly) is not a
  // company cost and must never reduce computed profit. deductions_total
  // (always >= 0) is added back so revenue-for-profit reflects charges only;
  // the client's billed amount (ventes.total_vente, clients.solde, client
  // statement) still uses total_vente + frais_total elsewhere, unchanged.
  const briqueSales      = briqueLivs.reduce((s, l) => s + (l.total_vente || 0) + (l.frais_total || 0) + (l.deductions_total || 0), 0)
  const grignonSales     = grignonLivs.reduce((s, l) => s + (l.total_vente || 0) + (l.frais_total || 0) + (l.deductions_total || 0), 0)
  ...
  const { cost: fuel, source: fuelSource } = computeFuelCost(voyage, voyageFuelMap, voyageGasoilLinks, pendingVoyageIds)
  const hasPendingFuel = fuelSource === 'pending'
  ...
  livraisons.forEach(l => {
    ...
    // Same déduction addback as briqueSales/grignonSales above.
    c.ventes += (l.total_vente || 0) + (l.frais_total || 0) + (l.deductions_total || 0)
  ...
  // Client Fuel Share must sum EXACTLY to the voyage's own fuel cost — the
  // same cent-exact largest-remainder split fuelAllocation.js already uses
  // for voyage-level allocation (distributeFuelPurchase), reused here with
  // brique quantity as the weight instead of distance. A plain `fuel * share`
  // per client (the previous formula) can leave a few cents unaccounted for
  // once each client's amount is independently rounded for display — exactly
  // the "unexplained difference" the reconciliation check must never show.
  // Grignon clients (briqueQte always 0) are excluded from the pool by
  // construction (distributeFuelPurchase only accepts distance/weight > 0),
  // so they always land on exactly 0 — never a Briques cost leaking onto a
  // Grignon client. When no brique client exists on this voyage at all
  // (100% Grignon), the pool is empty and nothing is distributed — that
  // portion of `fuel` is legitimately unattributable to any client, tracked
  // explicitly as `fuelUnallocated` below rather than silently vanishing, so
  // `fuel === Σ(client fuelAllocated) + fuelUnallocated` always holds exactly.
  const clientEntries = Array.from(cliMap.values())
  const fuelSplit = distributeFuelPurchase(fuel, clientEntries.map(c => ({ voyageId: c.key, distance: c.briqueQte })))
  const fuelAllocatedByKey = new Map(fuelSplit.map(s => [s.voyageId, s.amount]))
  const fuelUnallocated = Math.round((fuel - fuelSplit.reduce((s, f) => s + f.amount, 0)) * 100) / 100

  const clients = clientEntries.map(c => {
    const share             = briqueQtyShare(c.briqueQte, totalBriqueQteVoyage)
    const fuelAllocated      = fuelAllocatedByKey.get(c.key) || 0
    ...
    return {
      ...
      // A client's own fuel share is only as real as the voyage's fuel cost
      // itself — while the underlying refuel period is still open, this
      // client's fuelAllocated is 0 for the same reason the voyage's is,
      // never a fabricated split of an unknown total.
      hasPendingFuel,
    }
  })
  ...
  return {
    ...
    cost: {
      ...
      fuel, fuelSource, fuelUnallocated, hasPendingFuel, rental, chargesOperationnelles, total: costTotal,
    },
...
export function aggregateVoyageProfits(results = []) {
  ...
  const cost = {
    ...
    fuelUnallocated:        sum(r => r.cost.fuelUnallocated),
  ...
  // Never let a pending refuel period silently understate cost.fuel across an
  // aggregate — every consumer summing many voyages must be able to tell
  // "this total's Gasoil line is incomplete" (§10) from this count alone.
  const pendingFuelCount = results.filter(r => r.cost.hasPendingFuel).length
  return { revenue, cost, profit, marge, pendingFuelCount, warnings: results.flatMap(r => r.warnings) }
}
...
export function aggregateClientProfits(results = []) {
  ...
  results.forEach(r => {
    r.clients.forEach(c => {
      ...
      if (c.hasPendingFuel) acc.hasPendingFuel = true
