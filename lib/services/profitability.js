...
// Fuel cycle cost = diesel total + AdBlue total of the starting refill (g1) —
// AdBlue is bought together with diesel on the same plein, so it rides along
// with g1's cost the same way diesel does. adblue_total is 0/undefined on
// every plein that never records AdBlue, so this is a no-op then.
export function kmFuelCost(voyage, camionRefills) {
  ...
  const cycleCost = (g1.total || 0) + (g1.adblue_total || 0)
...
// sum of gasoil "pleins" manually linked to the voyage.
...
// Grouping key is `type_produit::type_brique_id` when both sides carry a
// type_brique_id (stable — survives a type_brique being renamed later), and
// falls back to the old `type_produit::type_brique` text match when either
// side lacks it (rows saved before type_brique_id existed). This keeps every
// historical voyage's WAC/profit numbers bit-identical while new saves match
// by ID going forward. Grignon achats/livraisons both use the literal
// type_brique 'Grignon' with no type_brique_id, so they always land in their
// own single bucket via the name-based key, same as before.
//
...
function wacTypeKey(type_produit, type_brique_id, type_brique) {
...
export function computeWACTable(achats, livraisons) {
  ...
  function ensure(type_produit, type_brique_id, type_brique) {
    const key = wacTypeKey(type_produit, type_brique_id, type_brique)
    if (!table.has(key)) {
      table.set(key, { type_produit, type_brique_id: type_brique_id || null, type_brique, achatCost: 0, achatQte: 0, deliveredQte: 0, wacPrice: null })
    }
    return table.get(key)
  }
  ;(achats || []).forEach(a => {
    const entry = ensure(a.type_produit, a.type_brique_id, a.type_brique)
  ...
  ;(livraisons || []).forEach(l => {
    const entry = table.get(wacTypeKey(l.type_produit, l.type_brique_id, l.type_brique))
...
export function costForDelivery(livraisonRow, wacTable) {
  const key = wacTypeKey(livraisonRow.type_produit, livraisonRow.type_brique_id, livraisonRow.type_brique)
