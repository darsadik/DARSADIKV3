...
// ── Cost-composition breakdown — presentation-only grouping of the exact same
// non-billed charge rows already summed into chargesOperationnelles above.
// transport/workers/other always reconstructs chargesOperationnelles exactly
// (other = total − workers − transport), so no total anywhere is affected —
// this only exposes what chargesOperationnelles is already made of, using the
// `categorie` field the caller already fetched (see CHARGE_CATS in
// lib/voyage-constants.js). Callers that don't fetch `categorie` simply see
// everything fall into `other`, which still reconciles correctly.
function categorizeOperationalCharges(charges) {
  const nonBilled = (charges || []).filter(c => !c.facture_client)
  const workers   = nonBilled.filter(c => c.categorie === 'ouvriers').reduce((s, c) => s + (c.montant || 0), 0)
  const transport = nonBilled.filter(c => c.categorie === 'transport').reduce((s, c) => s + (c.montant || 0), 0)
  const total     = nonBilled.reduce((s, c) => s + (c.montant || 0), 0)
  return { workers, transport, other: total - workers - transport }
}
...
export function computeVoyageProfit({
  ...
  const chargeCategories       = categorizeOperationalCharges(charges)
  ...
  const clients = Array.from(cliMap.values()).map(c => {
    ...
    // Same share applied per category — sums back to chargesAllocated exactly.
    const workersAllocated   = chargeCategories.workers * share
    const transportAllocated = chargeCategories.transport * share
    const otherAllocated     = chargeCategories.other * share
    ...
    return {
      ...
      cost: {
        achatWAC: c.achatWAC, fuelAllocated, rentalAllocated, chargesAllocated,
        workersAllocated, transportAllocated, otherAllocated, total: costTotalClient,
      },
  ...
  return {
    ...
    cost: {
      ...
      fuel, fuelSource, rental, chargesOperationnelles,
      chargesTransport: chargeCategories.transport, chargesWorkers: chargeCategories.workers, chargesOther: chargeCategories.other,
      total: costTotal,
    },
...
export function aggregateVoyageProfits(results = []) {
  ...
  const cost = {
    ...
    chargesTransport:       sum(r => r.cost.chargesTransport),
    chargesWorkers:         sum(r => r.cost.chargesWorkers),
    chargesOther:           sum(r => r.cost.chargesOther),
    ...
    r.clients.forEach(c => {
      if (!map.has(c.key)) {
        map.set(c.key, {
          ...
          cost: {
            achatWAC: 0, fuelAllocated: 0, rentalAllocated: 0, chargesAllocated: 0,
            workersAllocated: 0, transportAllocated: 0, otherAllocated: 0, total: 0,
          },
      ...
      acc.cost.workersAllocated     += c.cost.workersAllocated
      acc.cost.transportAllocated   += c.cost.transportAllocated
      acc.cost.otherAllocated       += c.cost.otherAllocated
