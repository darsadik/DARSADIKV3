...
export function explainRemaining(card) {
  ...
  if (card.voyageAllocations.length === 0) {
    // KM manquant, mais aucun voyage réel n'a pu être trouvé dans la période
    // (chronologique) associée à ce plein — distinct de "pas de KM du tout,
    // action manuelle requise" (resolvedBy null, ci-dessous): ici la période
    // EST établie, elle est simplement vide.
    if (!card.hasKm && card.resolvedBy === 'date') return 'Aucun voyage dans la période (associée par date) couverte par ce plein pour l\'instant.'
    ...
    table.allocations.forEach(a => {
      ...
      const voyageAllocations = a.voyageAllocations.map(va => {
        ...
        return {
          ...
          destination: row?.destination || voyage?.destination || null,
        }
      })
      ...
      cards.push({
        ...
        status: a.status,
        // 'km' (Priority 1, normal) | 'date' (Priority 2 — KM manquant sur ce
        // plein, période établie par chronologie) | null (aucune période
        // établissable — voir fuelAllocation.js's decision tree). Additive,
        // for a future/optional UI badge distinguishing the two ("Mesuré" vs
        // "KM manquant → associé par date") — see explainRemaining above for
        // the equivalent distinction already surfaced in the Remaining note.
        resolvedBy: a.resolvedBy,
        // Present only when this purchase absorbed earlier (or same-date)
        // km-less pleins' litres (Scenario C) — each one's own raw gasoil id/
        // date/qte, for transparency (spec item 12).
        carriedFrom: purchase.carriedFrom || null,
...
// ── Voyages needing fuel allocation (spec §2/§3/§14 case 1) ─────────────────
// A voyage with real KM/distance whose status isn't 'complete' /
// 'fuel_assigned_manually' is never silently "a real zero" — it's either
// still pending (its refuel period hasn't closed yet — fuelAllocation.js)
// or genuinely missing (no automatic bracket and no manual link). Reuses
// voyageRows exactly as buildVoyageKmFuelTimeline already computed them
// (status/destination/clientNames already resolved by the Chronologie tab's
// own loader) — no new calculation, just a filter + reshape so this state is
// visible directly in the Contrôle Allocation tab, not only as a Chronologie
// status badge.
export function buildUnallocatedVoyages(voyageRows) {
  return (voyageRows || [])
    .filter(r => r.distance !== null && r.distance > 0)
    .filter(r => r.status === 'pending_measurement' || r.status === 'fuel_not_assigned')
    .map(r => ({
      voyageId: r.voyageId,
      ...
      distance: r.distance,
      status: r.status, // 'pending_measurement' | 'fuel_not_assigned'
      // Provisional truck-average estimate (voyageKmFuel.js's isProvisional)
      // — still needs a real allocation once the next Plein closes the
      // period, so it stays in THIS list; only carried through so the panel
      // can show an indicative ≈figure instead of a bare status badge.
      isProvisional: r.isProvisional, provisionalLiters: r.provisionalLiters, provisionalL100: r.provisionalL100,
    }))
    .sort((a, b) => (a.date || '') < (b.date || '') ? -1 : 1)
}

// ── Fleet-wide fuel coverage summary (spec §16) ──────────────────────────────
// Every figure is a plain count/sum over voyageRows, already computed by
// buildVoyageKmFuelTimeline — no engine call, no new math. `totalCost` only
// sums voyages with a REAL determined cost — a pending/missing voyage never
// silently contributes 0 as if it were a confirmed zero.
export function buildFuelCoverageSummary(voyageRows) {
  const rows = (voyageRows || []).filter(r => r.distance !== null && r.distance > 0)
  const pending = rows.filter(r => r.status === 'pending_measurement')
  const missing = rows.filter(r => r.status === 'fuel_not_assigned')
  const allocated = rows.filter(r => r.status === 'complete' || r.status === 'fuel_assigned_manually')
  const totalKm = rows.reduce((s, r) => s + (r.distance || 0), 0)
  const totalCost = allocated.reduce((s, r) => s + (r.fuelCost || 0), 0)
  return {
    totalVoyages: rows.length, totalKm, totalCost,
    nbAllocated: allocated.length, nbPending: pending.length, nbMissing: missing.length,
  }
}
