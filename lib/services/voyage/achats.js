...
export async function delAchat(row) {
  ...
  if (row.grignon_operation_id) {
    ...
    // voyage_id-scoped so two voyages buying the same qty from the same
    // fournisseur on the same date can't cross-match.
    await supabase.from('grignon_operations').delete()
      .eq('fournisseur_id', row.fournisseur_id).eq('date', row.date_achat).eq('qte', row.qte).is('client_id', null).eq('voyage_id', row.voyage_id)
  }
}
