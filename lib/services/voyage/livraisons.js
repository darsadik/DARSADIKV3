...
export async function saveLiv(voyageId, form, { clients, grignonClients, typeBriques, voyage, achats }) {
  ...
  if (form.type_produit === 'brique') {
    const { error: rpcErr } = await supabase.rpc('save_livraison_brique', {
      ...
      p_frais:          fraisItems.map(f => ({ label: f.label, montant: parseFloat(f.montant) || 0, note: f.note || null })),
    })
      ...
      if (ld?.id) {
        ...
        if (fraisItems.length > 0) {
          await supabase.from('voyage_livraison_frais').insert(
            fraisItems.map(f => ({ livraison_id: ld.id, label: f.label, montant: parseFloat(f.montant) || 0, note: f.note || null }))
          )
        }
      }
