...
export async function saveLiv(voyageId, form, { clients, grignonClients, typeBriques, voyage, achats }) {
  ...
  // Step 1 — insert into ventes first (need the id for voyage_livraisons.vente_id)
  let venteId = null
  ...
  // Step 2 — insert voyage_livraisons (rollback ventes row if this fails)
  ...
  if (livErr) {
    // Rollback: delete the orphaned ventes row so the client statement stays clean
    if (venteId) await supabase.from('ventes').delete().eq('id', venteId)
    throw livErr
  }

  // Step 3 — write computed totals + frais via UPDATE (separate from INSERT to avoid GENERATED column conflicts)
  if (livData?.id) {
    await supabase.from('voyage_livraisons').update({ total_vente, total_achat, marge, frais_total }).eq('id', livData.id)
  }

  // Step 4 — save individual frais records
