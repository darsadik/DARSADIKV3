...
// ── Truck-specific historical average consumption — provisional-estimate
// support only (see lib/services/voyageKmFuel.js). Weighted: Σ(closed
// periods' real litres) ÷ Σ(closed periods' real distance) × 100 — NEVER a
// plain mean of each period's own L/100km ratio, so a 40km period and a
// 400km period don't count equally. `closedPeriods` must come straight from
// buildTruckFuelPeriods/buildFleetFuelPeriods above — every entry there is by
// construction a REAL refuel-to-refuel measurement, so an estimated/
// provisional figure computed downstream of this function can never feed
// back into it (nothing provisional is ever a "closed period").
export function truckHistoricalConsumption(closedPeriods) {
  const totalLitres = (closedPeriods || []).reduce((s, p) => s + (p.litresGasoil || 0), 0)
  const totalDistance = (closedPeriods || []).reduce((s, p) => s + (p.distance || 0), 0)
  return totalDistance > 0 ? (totalLitres * 100) / totalDistance : null
}

// Same weighted principle, DHS/km — used only to produce an INDICATIVE
// provisional cost figure alongside the estimated litres above. Never read
// by fuelAllocation.js/profitability.js's real cost engine.
export function truckHistoricalCostPerKm(closedPeriods) {
  const totalCost = (closedPeriods || []).reduce((s, p) => s + (p.coutTotal || 0), 0)
  const totalDistance = (closedPeriods || []).reduce((s, p) => s + (p.distance || 0), 0)
