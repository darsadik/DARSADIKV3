...
// buildPeriodSummary / buildFleetPeriodTotals, already computed once in
// pages/carburant/index.js) — nothing here recomputes distance, litres, or
// consumption. `trucks` is `[{ camion, currentKm, summary }, ...]`, the exact
// same shape the on-screen TruckControlCard renders, already scoped to
// Camions Propre and already filtered to the selected truck(s)/period.
...
function truckSection({ camion, currentKm, summary }) {
  const rows = [...summary.rowsInPeriod].sort((a, b) => a.date < b.date ? -1 : a.date > b.date ? 1 : 0)
