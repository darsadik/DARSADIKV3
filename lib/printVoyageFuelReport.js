...
// A4 LANDSCAPE, matching those same two sibling reports: 7 columns (Date,
// Destination, KM, Gasoil, L/100km, Coût, Plein utilisé — no Voyage
// reference column, by request) need the extra width portrait couldn't
// give Destination/KM without cramping them against each other.
//
...
// Navy stays for design/header elements (titlebar, table header, titles)
// only — actual voyage-row data (date/voyage/destination/km/gasoil/l100/
// coût/plein) uses this near-black instead, for maximum print contrast.
...
// "DD/MM" — same YYYY-MM-DD parsing as fmtDate, year dropped: the Plein
// utilisé column identifies a Plein by its own date now, not its gasoil.id.
...
// buildVoyageKmFuelTimeline). Identified by the Plein's own date (DD/MM,
// `c.date`) rather than its gasoil.id — easier for a reader to recognize at
// a glance — next to that Plein's own original quantity (`purchaseQte`) —
// never the voyage's own consumption, and never a fabricated value.
// `resolvedBy === 'date'` (fuelAllocation.js's Priority 2 fallback) means
// that Plein had no KM of its own, so it's flagged inline rather than
// silently shown as a normal match.
function pleinLabel(c) {
  const flag = c.resolvedBy === 'date' ? ' <span class="plein-flag">(KM manquant)</span>' : ''
  return `<div class="plein-item"><b>${fmtShortDate(c.date)}</b> — ${fmtMoney(c.purchaseQte)} Litres${flag}</div>`
}
...
function voyageRow(row) {
  ...
  const km = row.distance !== null && row.distance !== undefined ? `${fmt(row.distance)} KM` : '—'
  const hasLiters = hasRealLiters(row)
  const showProvisional = row.isProvisional && !hasLiters
  const gasoil = hasLiters ? `${fmtMoney(row.litersLinked)} L` : showProvisional ? `≈${fmtMoney(row.provisionalLiters)} L` : '—'
  const conso = hasLiters && row.distance > 0 ? consoStr((row.litersLinked / row.distance) * 100)
    : showProvisional ? `≈${consoStr(row.provisionalL100)}` : '—'
  const cout = row.fuelCost !== null && row.fuelCost !== undefined && row.fuelCost > 0 ? `${fmtMoney(row.fuelCost)} DH`
    : showProvisional && row.provisionalCost !== null ? `≈${fmtMoney(row.provisionalCost)} DH` : '—'
  const noteCls = st.tone === 'date' ? 'note-date' : st.tone === 'missing' ? 'note-missing' : st.tone === 'provisional' ? 'note-provisional' : st.tone === 'pending' ? 'note-pending' : ''
  // Provisional gets its own subtle neutral-gray row tint — deliberately
  // NOT the warm amber "band" used for warnings/missing data, so a reader
  // never confuses "estimated" with "needs attention".
  const rowCls = showProvisional ? 'provisional' : (st.tone !== 'ok' ? 'band' : '')
...
function truckSection({ camion, rows, from, to }) {
  ...
  <div class="truck-section">
    ...
    <table>
      <thead><tr>
        <th class="col-date">Date</th><th class="col-dest">Destination</th>
...
body{color:${NAVY};font-size:13.5px}
.report-title{margin:12px 8px 0;text-align:center}
...
.report-meta{display:flex;justify-content:center;gap:18px;flex-wrap:wrap;margin:8px 8px 0;padding-bottom:8px;border-bottom:1px solid ${LINE};font-size:11px;color:${MUTED}}
.report-meta b{color:${NAVY};font-weight:700}

.truck-section{margin:16px 8px 0}
...
.truck-section thead{display:table-header-group}
.truck-section thead th{background:${NAVY} !important;color:#fff !important;font-size:13.5px;font-weight:800;padding:11px 14px;white-space:nowrap}
.truck-section th.col-date{width:9%}
.truck-section th.col-dest{width:28%}
.truck-section th.col-km{width:9%}
.truck-section th.col-gasoil{width:10%}
.truck-section th.col-l100{width:10%}
.truck-section th.col-cout{width:14%}
.truck-section th.col-plein{width:20%}
.truck-section tbody td{font-size:14.5px;font-weight:700;color:${TEXT_DARK};padding:11px 14px;line-height:1.4;overflow-wrap:anywhere}
.truck-section td.dest{font-weight:700}
.truck-section td.dest .sub{font-size:10.5px;font-weight:600;color:${MUTED};margin-top:2px}
.truck-section tbody td.r{font-weight:800;font-family:'Courier New',monospace;white-space:nowrap}
.truck-section tbody td.r.l100{white-space:nowrap}
.truck-section tbody tr:nth-child(even) td{background:${BG_SOFT}}
.truck-section tr.band td{background:#fdf6ec !important}
.truck-section tr.provisional td{background:#f1f2f4 !important}
.note{font-family:Arial,sans-serif;font-weight:700;font-size:10px;margin-top:2px;white-space:normal}
.note-date{color:${WARN_COLOR}}
.note-missing{color:${BAD_COLOR}}
.note-pending{color:${MUTED};font-style:italic}
.note-provisional{color:${MUTED};font-style:italic}
.truck-section td.plein{font-size:13.5px;font-weight:700}
.plein-item + .plein-item{margin-top:4px}
.plein-item b{font-weight:800}
.plein-flag{font-size:10px;font-weight:700;color:${WARN_COLOR};white-space:normal}
