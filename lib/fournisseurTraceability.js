...
// Keys (ok/reste/depassement) are unchanged — used as lookups throughout the
// codebase. Only the visible emoji/label/color follow the requested
// 🟢 MATCH / 🟠 PARTIAL / 🔴 MISMATCH taxonomy.
export const TRACE_STATUS_META = {
  ok:          { emoji: '🟢', label: 'MATCH',    color: '#16a34a', bg: 'bg-emerald-50', text: 'text-emerald-600', ring: 'ring-emerald-100' },
  reste:       { emoji: '🟠', label: 'PARTIAL',  color: '#d97706', bg: 'bg-amber-50',   text: 'text-amber-600',   ring: 'ring-amber-100' },
  depassement: { emoji: '🔴', label: 'MISMATCH', color: '#dc2626', bg: 'bg-red-50',     text: 'text-red-600',     ring: 'ring-red-100' },
}

// Statut par ligne d'achat (utilisé dans la table Achats Briques) — distingue
// "rien distribué" de "dépassement", contrairement à traceStatus() qui ne
// connaît que reste/ok/depassement.
export function distributionStatus(qte, distribue) {
  const q = qte || 0
  const d = distribue || 0
  if (d > q + EPSILON) return 'depassement'
  if (Math.abs(d - q) < EPSILON) return 'fully'
  if (d <= EPSILON) return 'none'
  return 'partial'
}

export const DISTRIBUTION_STATUS_META = {
  fully:       { emoji: '✅', label: 'Entièrement distribué',   bg: 'bg-emerald-50', text: 'text-emerald-600' },
  partial:     { emoji: '⚠️', label: 'Partiellement distribué', bg: 'bg-amber-50',   text: 'text-amber-600' },
  none:        { emoji: '❌', label: 'Non distribué',           bg: 'bg-red-50',     text: 'text-red-600' },
  depassement: { emoji: '❌', label: 'Dépassement',             bg: 'bg-red-50',     text: 'text-red-600' },
}
...
export function buildAchatTraceability(voyageAchats, voyageLivraisons) {
  return (voyageAchats || []).map(a => {
    ...
    return {
      ...
      rowStatus: distributionStatus(a.qte, totalDistribue),
    }
  })
}
