...
// Same restrained corporate palette as lib/printControleKmCarburant.js —
// dark charcoal/navy for structure, ONE muted accent reserved for the
// consumption KPI, kept consistent across every fuel/gasoil PDF report.
const INK = '#1e293b'
const MUTED = '#64748b'
const ACCENT = '#9a3412'
...
.report-title{margin:16px 24px 0;font-size:18px;font-weight:800;letter-spacing:0.02em;color:${INK}}
.report-sub{margin:2px 24px 0;font-size:11px;color:${MUTED}}
tr.band td{background:#fdf6ec !important}
.footnote{font-size:10px;color:${MUTED};padding:8px 24px 4px;font-style:italic}
.grand-totals{margin:22px 24px 8px;border:1px solid ${INK};border-radius:6px;overflow:hidden;page-break-inside:avoid}
.gt-title{background:${INK};color:#fff;padding:9px 18px;font-size:12px;font-weight:800;letter-spacing:0.03em;text-transform:uppercase}
...
.gt-lbl{font-size:10px;font-weight:700;color:${MUTED};text-transform:uppercase;letter-spacing:0.05em}
.gt-val{font-size:19px;font-weight:800;color:#0f172a;margin-top:4px;font-family:'Courier New',monospace}
.gt-item.main .gt-val{color:${ACCENT}}
</style></head><body>
...
<div class="grand-totals">
  ...
  <div class="gt-grid">
    ...
    <div class="gt-item main"><div class="gt-lbl">Consommation moyenne pondérée</div><div class="gt-val">${fleetTotals.consoL100 !== null ? fleetTotals.consoL100.toFixed(2) + ' L/100km' : '—'}</div></div>
  </div>
</div>` : ''}
