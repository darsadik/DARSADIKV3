...
const VoyageDetailPanel = forwardRef(function VoyageDetailPanel({ voyageId, embedded = false, onSaved, onVoyageLoaded }, ref) {
  ...
  // instead of the whole fill — the same plein can then be linked again to
  // other voyages for the rest. Omitted/falsy = today's exact behavior
  // (whole plein, stamps gasoil.voyage_id as "claimed"). When splitting,
  // the source plein is intentionally left unstamped since several voyages
  // now share it — see sql/16_voyage_gasoil_split.sql.
  async function linkGasoilToVoyage(plein, splitQte) {
    ...
    if (!error) {
      // Whole-plein link only: mark the source plein as claimed. A split
      // link leaves it unstamped so it can still be found/linked again.
      if (!isSplit) await supabase.from('gasoil').update({ voyage_id: parseInt(id) }).eq('id', plein.id)
  ...
  // Explicit opt-in override (spec: "Automatic stays primary, splitting is
  // an override") — makes the fill(s) currently linked in voyage_gasoil the
  // voyage's actual fuel cost instead of the km-bracket automatic rate, via
  // the exact same updateFuelMode() write path FuelModeSection uses by hand.
  async function useLinkedGasoilAsFuelCost() {
    await saveFuelMode({
      fuelMode: 'manual_amount',
      manualDistanceKm: voyage?.manual_distance_km,
      manualCostPerKm:  voyage?.manual_cost_per_km,
      manualFuelCost:   totalGasoilManuel,
    })
  }
  ...
  // "Help entering KM" — suggests this truck's last known odometer reading
  // (the chronologically-previous voyage's km_arrivee, or its km_depart if
  // that voyage is itself still waiting on its own next reading) as a
  // starting point when Start KM is blank. Purely a suggestion filled into
  // the input — never written on its own, the user still clicks Enregistrer.
  const suggestedStartKm = useMemo(() => {
    if (!voyage?.camion_id) return null
    const sorted = [...camionVoyagesForCycle]
      .filter(v => !v.deleted_at)
      .sort((a, b) => a.date_depart !== b.date_depart
        ? (a.date_depart < b.date_depart ? -1 : 1)
        : (a.id || 0) - (b.id || 0))
    const idx = sorted.findIndex(v => v.id === voyage.id)
    if (idx <= 0) return null
    const prev = sorted[idx - 1]
    const val = prev.km_arrivee ?? prev.km_depart
    return (val !== null && val !== undefined) ? parseFloat(val) : null
  }, [camionVoyagesForCycle, voyage?.id, voyage?.camion_id])
  ...
  async function delGasoil(row) {
    try {
      ...
      if (rpcErr) {
        // Fallback: manual with fresh-read camion. A split assignment
        // (row.is_split) only owns a PORTION of the source plein — the
        // plein and camion totals were already accounted for once at the
        // original /gasoil insert, so only the link row is removed; the
        // shared gasoil row and camion stats must never be touched here
        // (mirrors the RPC's own is_split branch).
      ...
      <div className={embedded ? undefined : 'flex gap-4 items-start'}
        ...
        <div className="flex-1 min-w-0 space-y-4">
                ...
                <div>
                  ...
                  {editingKm ? (
                    <div>
                      ...
                      {!kmForm.km_depart && suggestedStartKm !== null && (
                        <button type="button"
                          onClick={() => setKmForm({ ...kmForm, km_depart: String(suggestedStartKm) })}
                          className="block mt-1 text-[10px] text-blue-500 hover:underline">
                          Suggestion : {fmt(suggestedStartKm)} (dernier KM connu de ce camion)
                        </button>
                      )}
                    </div>
                  ) : (
          ...
          <GasoilSection
            ...
            voyageId={id}
            onUseAsFuelCost={useLinkedGasoilAsFuelCost}
          />
