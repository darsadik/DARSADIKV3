...
import { linkGasoilToVoyage as dbLinkGasoilToVoyage, unlinkGasoilFromVoyage } from '../../lib/services/voyage/gasoilLink'
...
const VoyageDetailPanel = forwardRef(function VoyageDetailPanel({ voyageId, embedded = false, onSaved, onVoyageLoaded }, ref) {
  ...
  // as a starting point when Start KM is blank. Purely a suggestion filled
  // into the input — never written on its own, the user still clicks
  // Enregistrer. Shared with the Voyage KM & Fuel Manager (lib/services/kilometrage.js).
  const suggestedStartKm = useMemo(() => {
    if (!voyage?.camion_id) return null
    return suggestPreviousKm(camionVoyagesForCycle, voyage.id)
