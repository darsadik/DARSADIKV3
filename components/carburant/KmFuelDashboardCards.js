...
// Spec §8 — the exact 6-card operational dashboard. Every count is a plain
...
// the SAME filter state the per-truck timelines below already read, so a
// card and what's visible can never disagree about what "Fuel Not Assigned"
// or "Manual Assignments" means.
export default function KmFuelDashboardCards({
  dashboard, pleinsNeedingAssignment, pleinsAssigned,
  activeStatus, activeShow, onSelectStatus, onSelectShow,
}) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-6">
      <Card emoji="🟡" label="KM départ manquant" value={dashboard.byStatus.missing_start_km || 0}
      ...
      <Card emoji="🟠" label="KM arrivée manquant" value={dashboard.byStatus.missing_end_km || 0}
        toneClass="text-orange-600" active={activeStatus === 'missing_end_km'} onClick={() => onSelectStatus('missing_end_km')} />
