...
export default function ByVoyageSection({ results, onOpenVoyage }) {
  const columns = [
    ...
    { key: 'achat', label: 'Achats', right: true, sortValue: r => r.cost.achatTotal, exportValue: r => Math.round(r.cost.achatTotal), render: r => <span className="text-red-400">−{fmt(r.cost.achatTotal)}</span> },
    { key: 'fuel', label: 'Carburant', right: true, sortValue: r => r.cost.fuel, exportValue: r => Math.round(r.cost.fuel), render: r => <span className="text-orange-400">−{fmt(r.cost.fuel)}</span> },
    { key: 'rental', label: 'Location', right: true, sortValue: r => r.cost.rental, exportValue: r => Math.round(r.cost.rental), render: r => r.cost.rental > 0 ? <span className="text-amber-500">−{fmt(r.cost.rental)}</span> : '—' },
    { key: 'opex', label: 'Charges', right: true, sortValue: r => r.cost.chargesOperationnelles, exportValue: r => Math.round(r.cost.chargesOperationnelles), render: r => <span className="text-red-400">−{fmt(r.cost.chargesOperationnelles)}</span> },
    ...
    return (
      <tfoot>
        <tr className="border-t-2 border-slate-300 bg-gradient-to-r from-slate-800 to-slate-900 text-white">
          ...
          <td className="py-3 px-3 text-right font-bold text-red-300">−{fmt(ach)}</td>
          <td className="py-3 px-3 text-right font-bold text-orange-300">−{fmt(fuel)}</td>
          <td className="py-3 px-3 text-right font-bold text-amber-300">−{fmt(rent)}</td>
          <td className="py-3 px-3 text-right font-bold text-red-300">−{fmt(opex)}</td>
          <td className={`py-3 px-3 text-right font-black text-lg ${profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{profit >= 0 ? '+' : ''}{fmt(profit)}</td>
