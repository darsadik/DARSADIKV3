...
export default function Retours() {
  ...
  function printRetours() {
    ...
    openPrintWindow(`<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Retours Transport — DAR SADIK</title>
<style>
  *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;box-sizing:border-box;margin:0;padding:0}
  ...
  .kpi-g{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px}
  ...
  .kpi.rd{border-left-color:#dc2626}.kpi.rd .val{color:#dc2626}
...
<div class="bdy">
<div class="ttl">Retours Transport</div>
<div class="sub">Période : ${fmtDate(filterFrom)} → ${fmtDate(filterTo)} &nbsp;·&nbsp; ${filtered.length} opération(s)</div>
<div class="kpi-g">
  <div class="kpi bl"><div class="lbl">Total revenus</div><div class="val">${fmt(totalMontant)} DHS</div></div>
  <div class="kpi gn"><div class="lbl">Encaissé</div><div class="val">${fmt(totalPaye)} DHS</div></div>
  <div class="kpi rd"><div class="lbl">Reste à encaisser</div><div class="val">${fmt(totalRestant)} DHS</div></div>
  <div class="kpi pr"><div class="lbl">Taux recouvrement</div><div class="val">${totalMontant>0?Math.round(totalPaye/totalMontant*100):0}%</div></div>
</div>
<div class="sec">Détail des retours (${filtered.length})</div>
<table><thead><tr>
  <th>Date</th><th>Client</th><th>Destination</th><th>Camion</th><th>Chauffeur</th>
  <th class="r">Montant DHS</th><th class="r">Payé DHS</th><th class="r">Restant DHS</th><th>Statut</th><th>Note</th>
</tr></thead>
<tbody>${rows||'<tr><td colspan="10" style="text-align:center;color:#aaa;padding:14px;font-style:italic">Aucun retour</td></tr>'}</tbody>
${filtered.length>0?`<tfoot><tr>
  <td colspan="5">TOTAL (${filtered.length})</td>
  <td class="r">${fmt(totalMontant)} DHS</td>
  <td class="r">${fmt(totalPaye)} DHS</td>
  <td class="r">${fmt(totalRestant)} DHS</td>
  <td colspan="2"></td>
</tr></tfoot>`:''}
</table>
