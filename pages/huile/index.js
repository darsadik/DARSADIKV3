...
export default function Huile() {
  ...
  async function loadAll() {
    ...
    const [{ data: ops }, { data: cl }, { data: ca }, { data: fo }] = await Promise.all([
      supabase.from('huile_operations').select('*').order('date', { ascending: true }),
  ...
  async function saveOperation(e) {
    ...
    await supabase.from('huile_operations').insert({
      date:            form.date,
    ...
    // update client solde if client is set
    if (cl && totVente > 0) {
      await supabase.from('clients').update({ solde: (cl.solde || 0) + totVente }).eq('id', cl.id)
    }
  ...
  async function deleteOperation(op) {
    ...
    await supabase.from('huile_operations').delete().eq('id', op.id)
    // revert client solde
    if (op.client_id && op.total_vente) {
      const cl = clients.find(c => c.id === op.client_id)
      if (cl) await supabase.from('clients').update({ solde: Math.max(0, (cl.solde || 0) - op.total_vente) }).eq('id', cl.id)
    }
    loadAll()
  }

  // ── filtering (always date ASC from DB) ──
  ...
  function printClientView() {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
      <title>Huile d'Olive — Ventes Clients</title>
      <style>${PRINT_CSS}</style></head><body>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div><h1>🫒 DAR SADIK — Huile d'Olive · Ventes Clients</h1>
  ...
  function printFournisseurView() {
    ...
    filtered.forEach(op => {
      ...
      if (!byFourn[f]) byFourn[f] = { days: {}, total_qte: 0, total_achat: 0 }
      byFourn[f].total_qte   += op.qte || 0
      byFourn[f].total_achat += op.total_achat || 0
      if (!byFourn[f].days[op.date]) byFourn[f].days[op.date] = { qte: 0, total: 0, prix: op.prix_achat }
      byFourn[f].days[op.date].qte   += op.qte || 0
      byFourn[f].days[op.date].total += op.total_achat || 0
    })
    ...
    const win = window.open('', '_blank')
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
      <title>Huile d'Olive — Achats Fournisseurs</title>
      <style>${PRINT_CSS}</style></head><body>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div><h1>🫒 DAR SADIK — Huile d'Olive · Achats Fournisseurs</h1>
        <div class="sub">Période: ${filterFrom} → ${filterTo} | Généré le ${new Date().toLocaleDateString('fr-MA')}</div></div>
        <button class="print-btn" onclick="window.print()">🖨️ Imprimer</button>
      </div>
  ...
  function printCamionView() {
    ...
    const win = window.open('', '_blank')
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
      <title>Huile d'Olive — Camions</title>
      <style>${PRINT_CSS}</style></head><body>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div><h1>🫒 DAR SADIK — Huile d'Olive · Suivi Camions</h1>
        <div class="sub">Période: ${filterFrom} → ${filterTo} | Généré le ${new Date().toLocaleDateString('fr-MA')}</div></div>
        <button class="print-btn" onclick="window.print()">🖨️ Imprimer</button>
      </div>
      ${sections || '<p style="color:#aaa">Aucune donnée pour cette période</p>'}
  ...
  function FournisseurView() {
    ...
    // group by fournisseur then by day
    const byFourn = {}
    filtered.forEach(op => {
      const f = op.fournisseur_nom || 'Sans fournisseur'
      if (!byFourn[f]) byFourn[f] = { days: {}, total_qte: 0, total_achat: 0, total_vente: 0, total_marge: 0 }
      byFourn[f].total_qte   += op.qte || 0
      byFourn[f].total_achat += op.total_achat || 0
      byFourn[f].total_vente += op.total_vente || 0
      byFourn[f].total_marge += op.marge || 0
      if (!byFourn[f].days[op.date]) byFourn[f].days[op.date] = { qte: 0, total_achat: 0, prix: op.prix_achat }
      byFourn[f].days[op.date].qte        += op.qte || 0
      byFourn[f].days[op.date].total_achat += op.total_achat || 0
    })
  ...
  // ──────────────────────────────────────────────────────────
  return (
    <Layout title="Huile d'Olive" subtitle="Suivi achats, ventes et transport">
      ...
      {view === 'saisie' && admin && (
        <div className="card mb-4">
          <h3 className="font-bold text-gray-900 mb-4">➕ Nouvelle opération huile</h3>
