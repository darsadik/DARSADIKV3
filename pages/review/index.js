...
export default function ReviewMode() {
  ...
  const rows = useMemo(() => voyages.map(v => {
    ...
    return { voyage: v, result, anomalies, status, clientNoms }
  ...
  // ── keyboard navigation: ← / → move through the filtered queue ──
  useEffect(() => {
  ...
  return (
    <Layout title="Mode Révision" subtitle="Audit mensuel des voyages">
      <div className="flex gap-4 items-stretch" style={{ height: 'calc(100vh - 7.5rem)' }}>
        ...
        {/* ── RIGHT: selected voyage, fully editable ── */}
        <div className="flex-1 min-w-0 overflow-y-auto space-y-3">
          {selectedRow && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-2.5 flex items-center justify-between gap-3 sticky top-0 z-30">
              <button onClick={() => prevRow && setSelectedId(prevRow.voyage.id)} disabled={!prevRow}
                className="text-sm font-bold text-slate-500 hover:text-slate-800 disabled:opacity-30 disabled:cursor-not-allowed px-3 py-1.5 rounded-lg hover:bg-slate-50 transition">
                ← Précédent
              </button>
              <div className="flex items-center gap-3">
                <span title={STATUS_LABEL[selectedRow.status]}>{STATUS_DOT[selectedRow.status]}</span>
                <span className="text-xs text-slate-500 font-semibold">{selIdx + 1} / {filteredRows.length}</span>
          ...
          {selectedRow?.anomalies.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-2xl px-4 py-2.5 text-xs text-red-600 font-semibold space-y-0.5">
              {selectedRow.anomalies.map(a => <div key={a.type}>⚠ {a.label}</div>)}
            </div>
          )}

          {selectedId
            ? <VoyageDetailPanel voyageId={selectedId} embedded onSaved={patchRow} />
